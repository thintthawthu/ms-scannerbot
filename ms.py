...
def prompt_config():
    ...
    while True:
        ...
        if not token:
            print("❌ Token မထည့်ရင် မရပါ")
            continue
        if ":" not in token:
            print("❌ Token ပုံစံမှားနေပါ (123456:ABC-DEF... ပုံစံဖြစ်ရမယ်)")
            continue
        BOT_TOKEN = token
        break

    while True:
        admin = input("👤 Enter Admin ID (numeric): ").strip()
        if not admin:
            print("❌ Admin ID မထည့်ရင် မရပါ")
            continue
        if not admin.isdigit():
            print("❌ Admin ID က ဂဏန်းသက်သက် ဖြစ်ရပါမယ်")
            continue
        ADMIN_ID = int(admin)
        break
...
def save_hits():
    try:
        tmp_file = HITS_FILE + ".tmp"
        with open(tmp_file, "w", encoding="utf-8") as f:
            json.dump({str(k): v for k, v in success_texts.items()}, f, indent=2, ensure_ascii=False)
        os.replace(tmp_file, HITS_FILE)
    except Exception as e:
    ...
    try:
        ...
        for k, v in payload.items():
            ...
            if cid not in success_texts:
                success_texts[cid] = v
                count += len(v)
        print(f"[load_hits] Loaded {count} hits from file")
    except Exception as e:
...
def register_handlers():
    ...
    async def btn_start_scan(message):
        ...
        if not is_approved(chat_id):
            if USE_KEY_SYSTEM:
                await bot.send_message(chat_id, "🔑 /key ဖြင့် အတည်ပြုပါ")
            else:
                approve[chat_id] = True
                save_state()
            return
    ...
    @bot.message_handler(func=lambda msg: msg.text == "🛑 Stop Scan")
    async def btn_stop_scan(message):
        chat_id = message.chat.id
        ...
        if data and not data["task"].done():
            ...
            await bot.send_message(chat_id, "🛑 Scan ရပ်တန့်ပြီးပါပြီ", reply_markup=_main_keyboard())
        else:
    ...
    async def btn_success_codes(message):
        ...
        if not success and not limited:
            await bot.send_message(chat_id, "📭 ရှာတွေ့ထားသော code မရှိသေးပါ", reply_markup=_main_keyboard())
            return
    ...
    async def btn_help(message):
        ...
        await bot.send_message(message.chat.id, help_text, reply_markup=_main_keyboard())
    ...
    async def handle_input(message):
        ...
        if await check_session_url(url):
            ...
            await bot.send_message(message.chat.id, "❌ Session URL မှားယွင်းနေပါသည်။", reply_markup=_main_keyboard())
    ...
    async def smart_input(message):
        ...
        if text.startswith("http") and ("ruijienetworks.com" in text or "portal" in text.lower()):
            if await check_session_url(text):
                ...
                await bot.send_message(chat_id, "❌ Invalid Session URL", reply_markup=_main_keyboard())
            return
...
async def main():
    ...
    print("=" * 50)
    print("  MS TEAM — WiFi Voucher Scanner")
